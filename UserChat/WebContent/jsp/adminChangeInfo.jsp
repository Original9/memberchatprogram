		...
		$(function() {
			...
			$("body").on(
					...
					function() {
						...
						var genVal = td.children().eq(4).text();
						var gradeVal = td.children().eq(6).text();

						console.log(genVal);
						console.log(gradeVal);
						console.log(td.children().eq(0).text());

						document.getElementById("id").value = td.children().eq(
								0).text();
						document.getElementById("password").value = td
								.children().eq(1).text();
						document.getElementById("name").value = td.children()
								.eq(2).text();
						document.getElementById("age").value = td.children()
								.eq(3).text();
						$("#selectGender").val(genVal).attr("selected",
								"selected");
						document.getElementById("email").value = td.children()
								.eq(5).text();
						$("#selectGrade").val(gradeVal).attr("selected",
								"selected");
		...
		<table id="userList" class="table table-striped table-bordered"
			...
			<tbody>
				...
				<c:forEach items="${list }" var="dto">
					<tr>
						<%-- onclick="location.href='#.do?key=${dto.userID}'" 클릭 시 #.do로 키값 넘기는 옵션--%>
						...
						<td align="center"><button id="update-user${status.count}"
								class="btn btn-primary">수정</button>&nbsp;&nbsp;&nbsp;&nbsp;
							<button id="delete-user${status.count}" class="btn btn-primary">삭제</button></td>
					</tr>
				</c:forEach>
		...
		<form name="frm" action="adminUpdateUser.do">
			<fieldset>
				<label for="id">ID</label> <input type="text" name="userID" id="id"
					...
					readOnly="readOnly"> <label for="password">Password</label>
				...
				<label for="email">Email</label> <input type="text" name="userEmail"
					id="email" value="" class="text ui-widget-content ui-corner-all">
				<label for="age">Age</label>
				<input type="text" name="userAge" id="age"	value="" 
					class="text ui-widget-content ui-corner-all">
				<div class="row">
				<div class="col-sm-3">
				<label for="gender">Gender</label>
					<select id="selectGender" name="userGender"
				...
				<select id="selectGrade"
					style="width: 80px" class="form-control" name="userGrant">
					<option>U</option>
					<option>M</option>
					</select>
				</div>
